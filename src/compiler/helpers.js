...
export function addHandler (
  ...
  if (modifiers.right) { // 右键 
    if (dynamic) {
      name = `(${name})==='click'?'contextmenu':(${name})` // 动态属性
    } else if (name === 'click') {
      name = 'contextmenu' // 非动态属性，name = contextmenu
      delete modifiers.right // 删除修饰符中的 right 属性
    }
  } else if (modifiers.middle) {
    if (dynamic) { // 中间键
      name = `(${name})==='click'?'mouseup':(${name})` // 动态属性，name => mouseup 或者 ${name}
    } else if (name === 'click') {
      name = 'mouseup' // 非动态属性，mouseup
    }
  }
