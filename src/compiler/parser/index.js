...
export function parse (
  ...
  platformMustUseProp = options.mustUseProp || no // 必须使用 props 进行绑定的属性
  platformGetTagNamespace = options.getTagNamespace || no // 获取标签的命名空间
  const isReservedTag = options.isReservedTag || no // 是否是保留标签（html + svg)
  maybeComponent = (el: ASTElement) => !!el.component || !isReservedTag(el.tag) // 判断一个元素是否为一个组件
  ...
  function closeElement (element) {
    trimEndingWhitespace(element) // 移除节点末尾的空格，当前 pre 标签内的元素除外
  ...
  parseHTML(template, {
    ...
    */
      ...
      if (inVPre) {
        ...
        // 结构化的指令
        // v-for
        processFor(element) // 处理 v-for 属性，得到 element.for = 可迭代对象 element.alias = 别名
          /**
         ...
         */
        processIf(element)
        processOnce(element) // 处理 v-once 指令，得到 element.once = true 
      }
...
export function processElement (
  ...
  processRef(element)
  processSlotContent(element) // 处理作为插槽传递给组件的内容，得到  插槽名称、是否为动态插槽、作用域插槽的值，以及插槽中的所有子元素，子元素放到插槽对象的 children 属性中
  processSlotOutlet(element) // 处理自闭合的 slot 标签，得到插槽名称 => el.slotName = xx
...
function processKey (el) {
  const exp = getBindingAttr(el, 'key') // 拿到 key 的属性值
  if (exp) {
    // 关于 key 使用上的异常处理
    if (process.env.NODE_ENV !== 'production') {
      if (el.tag === 'template') { // template 标签不允许设置 key
        warn(
          `<template> cannot be keyed. Place the key on real elements instead.`,
          getRawBindingAttr(el, 'key')
        )
      }
      if (el.for) { // 不要在 <transition=group> 的子元素上使用 v-for 的 index 作为 key，这和没用 key 没什么区别
  ...
  if (process.env.NEW_SLOT_SYNTAX) {
    if (el.tag === 'template') {
      ...
      if (slotBinding) {
        ...
        el.slotTarget = name // 得到插槽名称
        el.slotTargetDynamic = dynamic // 是否为动态插槽
        el.slotScope = slotBinding.value || emptySlotScopeToken // force it into a scoped slot for perf// 作用域插槽的值
      }
    } else {
      ...
      if (slotBinding) {
        ...
        const slots = el.scopedSlots || (el.scopedSlots = {}) // 将组件的孩子添加到它的默认插槽内
        const { name, dynamic } = getSlotName(slotBinding)  // 获取插槽名称以及是否为动态插槽
        // 创建一个 template 标签的 ast 对象，用于容纳插槽内容，父级是 el
        const slotContainer = slots[name] = createASTElement('template', [], el)
        slotContainer.slotTarget = name // 插槽名
        slotContainer.slotTargetDynamic = dynamic // 是否为动态插槽
...
function processSlotOutlet (el) {
  if (el.tag === 'slot') {
    el.slotName = getBindingAttr(el, 'name') // 得到插槽名称
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) { // 说明该属性是一个指令
      ...
      if (bindRE.test(name)) { // v-bind, <div :id="test"></div>
        ...
        name = name.replace(bindRE, '') // 属性名，比如：id
        value = parseFilters(value) // 属性值，比如：test
        isDynamic = dynamicArgRE.test(name) // 是否为动态属性 <div :[id]="test"></div>
        if (isDynamic) {
          name = name.slice(1, -1) // 如果是动态属性，则去掉属性两侧的方括号 []
        }
         // 提示，动态属性值不能为空字符串
        if (
        ...
        name = name.replace(onRE, '') // 属性名，即事件名
        isDynamic = dynamicArgRE.test(name) // 是否为动态属性
        if (isDynamic) {
          name = name.slice(1, -1) // 动态属性，则获取 [] 中的属性名
        }
        // 处理事件属性，将属性的信息添加到 el.events 或者 el.nativeEvents 对象上，格式：
        // el.events = [{ value, start, end, modifiers, dynamic }, ...]
        addHandler(el, name, value, modifiers, false, warn, list[i], isDynamic)
      } else { // normal directives其它的普通指令
