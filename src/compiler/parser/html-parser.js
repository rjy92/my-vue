...
export function parseHTML (html, options) {
  ...
  const isUnaryTag = options.isUnaryTag || no // 是否是自闭合标签
  const canBeLeftOpenTag = options.canBeLeftOpenTag || no // 是否可以只有开始标签
  let index = 0 // 记录当前在原始 html 字符串中的开始位置
  let last, lastTag
  while (html) {
    last = html // 确保不是在 script、style、textarea 这样的纯文本元素中
    // Make sure we're not in a plaintext content element like script/style
    if (!lastTag || !isPlainTextElement(lastTag)) {
      let textEnd = html.indexOf('<') // 找第一个 < 字符
      // textEnd === 0 说明在开头找到了
